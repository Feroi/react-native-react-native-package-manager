...
public class RNReactNativePackageManagerModule extends ReactContextBaseJavaModule {
    ...
    public static Map <String, UsageStats> getAggregateStatsMap(Context context, long startDate, long endDate) {
    ...
    public void getStats(
        ...
        try {
            ...
            String stats = getStatsString(getAggregateStatsMap(reactContext, start, end));
    ...
    @ReactMethod
    public void getFirstInstallApp(Promise promise) {
        long installed;
        try {
            installed = reactContext.getPackageManager().getPackageInfo(reactContext.getPackageName(), 0).firstInstallTime;
        } catch (PackageManager.NameNotFoundException e) {
            installed = Calendar.getInstance().getTimeInMillis();
        }

        promise.resolve(String.valueOf(installed));
    }
